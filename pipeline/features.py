...
def hand_features(hand: dict) -> Dict[str, float]:
    ...
    # Hero-conditioned features (v4/H3): the label is about ONE player
    # (metadata.hero_seat), so isolate that player's behavior from opponents'.
    hero = int(meta.get("hero_seat") or 0)
    hero_actions = [a for a, s in zip(actions, actors) if s == hero]
    hero_types = [a.get("action_type", "") for a in hero_actions]
    hc = Counter(hero_types)
    nh = max(len(hero_actions), 1)
    f["hero_action_share"] = len(hero_actions) / n_act
    f["hero_zero_actions"] = float(not hero_actions)
    for t in _ACTIONS:
        f[f"hero_share_{t}"] = hc.get(t, 0) / nh
    hero_aggr = hc.get("bet", 0) + hc.get("raise", 0)
    f["hero_aggr_share"] = hero_aggr / nh
    f["hero_vpip"] = float(any(t in ("call", "bet", "raise") for t in hero_types))
    f["hero_folded"] = float("fold" in hero_types)
    f["hero_n_actions"] = float(len(hero_actions))
    hero_amts = [
        float(a.get("normalized_amount_bb") or 0.0)
        for a in hero_actions
        if float(a.get("normalized_amount_bb") or 0.0) > 0
    ]
    f["hero_amount_mean_bb"] = float(np.mean(hero_amts)) if hero_amts else 0.0
    hero_bkts = [_bucket_index(a) for a in hero_amts]
    f["hero_bucket_mean"] = float(np.mean(hero_bkts)) if hero_bkts else 0.0
    f["hero_bucket_top_share"] = (
        max(Counter(hero_bkts).values()) / len(hero_bkts) if hero_bkts else 0.0
    )
    # hero bet sized relative to the pot it enters (shift-robust)
    hero_ratios = [
        float(a.get("normalized_amount_bb") or 0.0)
        / max(float(a.get("pot_before") or 0.0) / _VISIBLE_BB, 0.5)
        for a in hero_actions
        if float(a.get("normalized_amount_bb") or 0.0) > 0
    ]
    f["hero_amt_pot_ratio_mean"] = float(np.mean(hero_ratios)) if hero_ratios else 0.0
    f["hero_entropy_action"] = _entropy(hero_types)

    return f


def _signatures(hand: dict) -> Dict[str, str]:
    """Windowing-robust canonical hand keys (v4/M2).

    The validator shows only a deterministically SAMPLED 5-8 action window per
    hand, and the sample differs per hand, so exact ordered action strings almost
    never repeat across a stationary policy's hands. These keys are order- and
    sample-robust: two hands from the same policy collide far more often, so the
    chunk-level repetition/entropy signal actually survives sanitization.
    """
    actions = hand.get("actions") or []
    types = [a.get("action_type", "") for a in actions]
    hero = int((hand.get("metadata") or {}).get("hero_seat") or 0)
    hero_types = [
        a.get("action_type", "")
        for a in actions
        if int(a.get("actor_seat") or 0) == hero
    ]
    reached_river = any(
        (a.get("street") or "") == "river" for a in actions
    )
    # multiset (order-invariant) of action types
    action_ms = "|".join(sorted(types))
    # per-street action-type multiset — captures policy shape, not exact order
    by_street: Dict[str, List[str]] = {}
    for a in actions:
        by_street.setdefault(str(a.get("street") or "p"), []).append(
            a.get("action_type", "")
        )
    street_ms = ";".join(
        f"{s}:{','.join(sorted(v))}" for s, v in sorted(by_street.items())
    )
    # coarse hand "shape": how many actions, dominant action, depth, hero fold
    dom = Counter(types).most_common(1)[0][0] if types else ""
    shape = f"{min(len(actions), 9)}_{dom}_{int(reached_river)}_{int('fold' in hero_types)}"
    # hero's own action multiset (target-player line, sample-robust)
    hero_ms = "|".join(sorted(hero_types))
    return {
        "action_sig": action_ms,
        "street_sig": street_ms,
        "shape_sig": shape,
        "hero_sig": hero_ms,
    }
...
def chunk_features(hands: List[dict]) -> Dict[str, float]:
    ...
    for name in ("action_sig", "street_sig", "shape_sig", "hero_sig"):
        keys = [s[name] for s in sigs]
        c = Counter(keys)
        # top_share and normalized entropy are size-robust; unique_share shrinks
        # with n for a stationary policy, so we keep it but lean on entropy.
        out[f"{name}_top_share"] = max(c.values()) / n if c else 0.0
        out[f"{name}_unique_share"] = len(c) / n if c else 0.0
        ent = _entropy(keys)
        out[f"{name}_entropy"] = ent
        out[f"{name}_norm_entropy"] = ent / math.log2(n) if n > 1 else 0.0
