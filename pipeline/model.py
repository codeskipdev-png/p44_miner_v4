...
# v4/M1: capacity cut + stronger regularization. ~2k training rows can't support
# 500-leaf-96 trees without overfitting, and an overfit model transfers worse to
# the live shift. Fewer/shallower trees, larger leaves, explicit L2.
    ...
    return StackingClassifier(
        estimators=[
            ("lgbm", LGBMClassifier(
                ...
                reg_lambda=2.0, random_state=seed, verbose=-1, n_jobs=4)),
            ("xgb", XGBClassifier(
                n_estimators=300, max_depth=4, grow_policy="depthwise",
                learning_rate=0.03, subsample=0.8, colsample_bytree=0.6,
                min_child_weight=5.0, reg_lambda=2.0,
                random_state=seed, n_jobs=4, eval_metric="logloss")),
            ("cat", CatBoostClassifier(
                iterations=400, depth=5, learning_rate=0.04, l2_leaf_reg=6.0,
                random_seed=seed, verbose=0, thread_count=4)),
            ("et", ExtraTreesClassifier(
                n_estimators=300, max_depth=12, min_samples_leaf=8,
                random_state=seed, n_jobs=4)),
        ],
        final_estimator=LogisticRegression(C=0.3, max_iter=2000),
...
def build_mono(signs: List[int]) -> VotingClassifier:
    members = [
        (f"mlgb{s}", LGBMClassifier(
            n_estimators=300, num_leaves=31, learning_rate=0.035,
            subsample=0.85, colsample_bytree=0.7, min_child_samples=30,
            reg_lambda=2.0, monotone_constraints=signs, random_state=100 + s,
            verbose=-1, n_jobs=4))
        for s in range(3)
    ]
...
def build_mlp() -> VotingClassifier:
    members = [
        (f"mlp{s}", Pipeline([
            ("sc", StandardScaler()),
            ("pca", PCA(n_components=40, random_state=200 + s)),
            ("mlp", MLPClassifier(
                hidden_layer_sizes=(64,), alpha=1e-2, max_iter=600,
                early_stopping=True, n_iter_no_change=15, random_state=200 + s)),
        ]))
        for s in range(3)
    ]
